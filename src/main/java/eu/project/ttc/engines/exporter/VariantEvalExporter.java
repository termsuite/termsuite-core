...
public class VariantEvalExporter extends AbstractTermIndexExporter {
	...
	protected void processAcceptedTerms(TreeSet<Term> acceptedTerms)
		...
		try {
			...
			for(Term t:acceptedTerms) {
				...
				printBase(++rank, t);
				int variantRank = 0;
				for(SyntacticVariation variation:t.getSyntacticVariants()) {
					if(variantRank >= nbVariantsPerTerm)
						break;
					variantCnt++;
					variantRank++;
					printVariation(rank, variantRank, variation);
					printTermOccurrences(variation.getTarget());
				}
	...
	private void printVariation(int termRank, int variantRank, SyntacticVariation variation) throws IOException {
		Term variant = variation.getTarget();
		...
		writer.write("\t");
		writer.write(String.format("<%s>", variation.getVariationRule()));
		writer.write("\t");
